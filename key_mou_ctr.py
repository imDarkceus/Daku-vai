...
async def scroll_cursor_tool(direction: str, amount: int = 10):
    ...
    Useful for commands like "scroll down" or "উপর কর".
...
async def type_text_tool(text: str):
    ...
    Useful for commands like "type hello world" or "hello লেখ".
...
async def press_key_tool(key: str):
    ...
    Useful for commands like "Enter চাপুন", "Escape চাপুন", or "A press কর".
...
async def press_hotkey_tool(keys: List[str]):
    ...
    Use this when the user says something like "save কর", "window বন্ধ কর",
    or "refresh করে দাও".
...
async def control_volume_tool(action: str):
    ...
    Use this when the user says something like "volume বাড়ান", "mute করে দাও",
    or "lower the sound".
...
async def swipe_gesture_tool(direction: str):
    ...
    for example: "নিচে স্ক্রোল কর", "left swipe কর", or "screen উপরে কর".
