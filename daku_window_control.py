...
async def open_folder(path):
    try:
        ...
        logger.error(f"❌ ফাইল খুলতে error হয়েছে। {e}")
...
async def create_folder(path):
    try:
        ...
        return f"✅ Folder তৈরি হয়ে গেছে।: {path}"
    except Exception as e:
        return f"❌ ফাইল তৈরি করতে error হয়েছে।: {e}"
...
async def open_app(app_title: str) -> str:
    ...
    - "Notepad খোল"
    - "Chrome open কর"
    - "VLC media player চালাও"
    - "Calculator launch কর"
    """
    ...
    try:
        ...
        if focused:
            return f"🚀 App চালু হয়েছে এবং focus-এ আছে: {app_title}।"
        else:
            return f"🚀 {app_title} চালু হয়েছে, কিন্তু window-তে focus করা যায়নি।"
    except Exception as e:
...
async def close_app(window_title: str) -> str:
    ...
    - "Notepad বন্ধ কর"
    - "Close VLC"
    - "Chrome-এর window বন্ধ করে দাও"
    - "Calculator-কে বন্ধ কর"
    """
    ...
    return f"❌ Window বন্ধ হয়েছে।: {window_title}"
...
async def folder_file(command: str) -> str:
    ...
    Example prompts:
    - "Projects folder তৈরি কর"
    - "OldName-কে NewName-এ rename কর"
    - "xyz.mp4 delete করে দাও"
    - "Music folder খোল"
    - "Resume.pdf চালাও"
    """
    ...
    if "delete" in command_lower:
        ...
        return "❌ Delete করার জন্য কোনো item পাওয়া যায়নি।"
